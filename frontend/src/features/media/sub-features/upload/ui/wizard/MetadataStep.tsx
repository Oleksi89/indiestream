...
export const MetadataStep = () => {
    ...
    const md = t.media.upload.metadata;
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}
            ...
            <div>
                <label htmlFor="meta-title" className="block text-sm font-medium text-slate-300 mb-1.5">
                    {md.titleLabel} <span className="text-red-400">*</span>
                </label>
                <input
                    id="meta-title"
                    {...register('title')}
                    ...
                    placeholder={md.titlePlaceholder}
                />
                {errors.title && <p className="text-red-400 text-xs mt-1.5" role="alert">{errors.title.message}</p>}
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label htmlFor="meta-genre" className="block text-sm font-medium text-slate-300 mb-1.5">{md.genreLabel}</label>
                    <select
                        id="meta-genre"
                        {...register('genre')}
                    ...
                    >
                        <option value="0">{md.genrePlaceholder}</option>
                        {AVAILABLE_GENRES.map(g => (
                            <option key={g} value={g}>{g}</option>
                        ))}
                    </select>
                    {errors.genre && <p className="text-red-400 text-xs mt-1.5" role="alert">{errors.genre.message}</p>}
                </div>

                <div
                    className="flex items-center justify-between p-4 rounded-xl border border-slate-700 bg-slate-900/50">
                    <div>
                        <p className="text-sm font-medium text-slate-200">{md.explicitLabel}</p>
                        <p className="text-xs text-slate-500">{md.explicitDescription}</p>
                    </div>
                    <Controller
                        name="isExplicit"
                        control={control}
            ...
            <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">{md.tagsLabel}</label>
                <div
                    ...
                    {customTags.map(tag => (
                        <span key={tag}
                            ...
                            <Tag size={10} className="text-violet-400" aria-hidden="true"/>
                            {tag}
                            <button
                                type="button"
                                onClick={() => removeTag(tag)}
                                aria-label={md.removeTag.replace('{tag}', tag)}
                                title={md.removeTag.replace('{tag}', tag)}
                    ...
                    <input
                        ...
                        placeholder={customTags.length < 10 ? md.tagInputPlaceholder : md.tagsMaxReached}
                        disabled={customTags.length >= 10}
                        aria-label={md.tagsLabel}
                        className="flex-1 min-w-[140px] bg-transparent text-sm text-slate-100 placeholder-slate-500 outline-none px-2 py-1"
                    />
                </div>
                {errors.customTags && <p className="text-red-400 text-xs mt-1.5" role="alert">{errors.customTags.message}</p>}
                <p className="text-slate-500 text-[11px] mt-1.5">{md.tagsHint}</p>
            </div>
            ...
            <div className="flex justify-end pt-4">
                <button type="submit"
                    ...
                    {md.continueToMedia} <ArrowRight size={16} aria-hidden="true"/>
                </button>
            </div>
        </form>
    );
