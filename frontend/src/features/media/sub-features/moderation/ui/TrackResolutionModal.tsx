                ...
                <Dialog.Content
                    ...
                    <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <Dialog.Title className="text-lg font-semibold text-slate-100">AI Metadata
                                    Review</Dialog.Title>
                                <Dialog.Description
                                    className="text-sm text-slate-400">{track.title}</Dialog.Description>
                            </div>
                        </div>
                    ...
                    <div className="mb-6 p-4 rounded-xl border border-slate-700 bg-slate-800/50">
                        <p className="text-sm text-slate-300">
                            Our AI moderation engine has suggested semantic tags based on the audio profile.
                            Accepting these ensures maximum visibility in discovery feeds.
                        </p>
                    </div>

                    <div className="grid grid-cols-2 gap-4 mb-6">
                        <div className="p-4 rounded-xl border border-slate-700 bg-slate-950">
                            <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Your
                                Tags</h4>
                            <div className="flex flex-wrap gap-2">
                                {customTags.length ? customTags.map(t => (
                                    <span key={t}
                                          className="px-2 py-1 bg-slate-800 text-slate-300 text-xs rounded">{t}</span>
                        ...
                        <div className="p-4 rounded-xl border border-indigo-500/30 bg-indigo-500/5">
                            <h4 className="text-xs font-semibold text-indigo-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                                <Sparkles size={12}/> AI Suggestions</h4>
                            <div className="flex flex-wrap gap-2">
                                {aiTags.length ? aiTags.map(t => (
                                    <span key={t}
                                          className="px-2 py-1 bg-indigo-500/20 text-indigo-300 text-xs rounded">{t}</span>
                                )) : <span className="text-slate-500 text-xs">None</span>}
                            </div>
                        </div>
                    </div>
                    ...
                    {mode === 'VIEW' ? (
                        <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                            ...
                            >
                                {isAccepting ? 'Approving...' : 'Accept AI Suggestions'} <ArrowRight size={16}/>
                            </button>
                        </div>
                    ) : (
                        <form onSubmit={handlePropose} className="space-y-4 animate-in slide-in-from-bottom-2">
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-2">Justification for
                                    override <span className="text-red-400">*</span></label>
                                <textarea
                                    value={justification}
                                    onChange={(e) => setJustification(e.target.value)}
                                    placeholder="Explain why the AI tags are incorrect for your track..."
                            ...
                            <div className="flex justify-between items-center pt-4 border-t border-slate-800">
                                <div className="text-xs text-amber-400 flex items-center gap-1">
                                    <AlertCircle size={14}/> Overrides require Admin review (up to 48h).
                                </div>
                                <div className="flex gap-3">
                                    ...
                                    <button type="submit" disabled={isProposing || justification.length < 10}
                                        ...
                                        {isProposing ? 'Submitting...' : 'Submit for Review'}
                                    </button>
                                </div>
                            </div>
                        </form>
