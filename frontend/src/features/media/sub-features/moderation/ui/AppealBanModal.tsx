...
export const AppealBanModal = ({track, isOpen, onClose}: AppealBanModalProps) => {
    ...
    const ap = t.media.moderation.appeal;
                ...
                <Dialog.Content
                    ...
                    <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <Dialog.Title className="text-lg font-semibold text-slate-100">{ap.title}</Dialog.Title>
                                <Dialog.Description className="text-sm text-slate-400">
                                    {ap.trackLabel} {track.title}
                                </Dialog.Description>
                            </div>
                        </div>
                        <button
                    ...
                    <div
                        ...
                        role="alert">
                        <AlertTriangle size={18} className="shrink-0 text-amber-400" aria-hidden="true"/>
                        <p>{ap.warning}</p>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label htmlFor="appeal-reason" className="sr-only">{ap.title}</label>
                            <textarea
                                id="appeal-reason"
                                value={reason}
                            ...
                            <div className="flex justify-end mt-1">
                                <span className={`text-xs ${reason.length < 20 ? 'text-red-400' : 'text-emerald-400'}`}
                                      aria-live="polite">
                                    {ap.charCount.replace('{count}', String(reason.length))}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                            ...
                            >
                                {isPending ? ap.submitting : ap.submit}
                            </button>
                        </div>
                    </form>
                </Dialog.Content>
