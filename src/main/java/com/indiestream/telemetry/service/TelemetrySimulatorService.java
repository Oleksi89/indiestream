...
public class TelemetrySimulatorService {
     ...
     */
    public int purgeShadowTraffic(OffsetDateTime start, OffsetDateTime end) {
        ...
        // Housekeeping: Clean up quick-seed simulations that injected non-existent random user IDs
        StringBuilder sqlOrphanedInteractions = new StringBuilder(
                "DELETE FROM raw_interaction_logs WHERE user_id IS NOT NULL AND user_id NOT IN (SELECT id FROM users)"
        );
        StringBuilder sqlOrphanedPlaybacks = new StringBuilder(
                "DELETE FROM raw_playback_logs WHERE user_id IS NOT NULL AND user_id NOT IN (SELECT id FROM users)"
        );
        ...
        int orphanedInteractionsDeleted = jdbcTemplate.update(sqlOrphanedInteractions.toString(), params);
        int playbacksDeleted = jdbcTemplate.update(sqlPlaybacks.toString(), params);
        int orphanedPlaybacksDeleted = jdbcTemplate.update(sqlOrphanedInteractions.toString(), params);

        log.info("Purge complete. Deleted {} playbacks, {} simulator interactions, {}  orphaned interactions, {}  orphaned playbacks.",
                playbacksDeleted, interactionsDeleted, orphanedInteractionsDeleted, orphanedPlaybacksDeleted);

        return playbacksDeleted + interactionsDeleted + orphanedInteractionsDeleted + orphanedPlaybacksDeleted;
    }
