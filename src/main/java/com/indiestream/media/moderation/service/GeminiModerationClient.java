...
class GeminiModerationClient {
    ...
    private static final List<String> MODEL_FALLBACK_CHAIN = List.of(
            ...
            "gemini-2.5-flash",     // Tier 2: Balanced fallback (5 RPM / 20 RPD)
    ...
    private boolean isRateLimitException(Exception e) {
        ...
        String topMessage = e.getMessage() != null ? e.getMessage().toLowerCase() : "";

        return topMessage.contains("429") || rootMessage.contains("429")
                || topMessage.contains("quota") || rootMessage.contains("quota")
                || topMessage.contains("too many requests") || rootMessage.contains("too many requests");
    }
}
